...
from utils.ROI import left_camera, right_camera
...
def plot_one_box(x, img, color=None, label=None, line_thickness=3, is_left=1):
    # Get ROI
    if is_left:
    ...
    # -1: left - 0: not count - 1: right
    place = 0
    if (isInside(left_points, centroid)) == True:
      place = -1
    if (isInside(right_points, centroid)) == True:
      place = 1
